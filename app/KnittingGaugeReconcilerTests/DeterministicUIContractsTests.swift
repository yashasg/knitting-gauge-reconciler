...
struct DeterministicUIContractsTests {
    ...
    @Test func sceneDraftStoreRejectsMalformedDataAndCleansOnlyOwnedScenes() throws {
        ...
        let serialized = try #require(SceneDraftStore.serialize(values: values, disclosure: true))
        ...
        let suiteName = "DeterministicUIContractsTests.\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
        SceneDraftStore.save(serialized, sceneID: "one", defaults: defaults)
        #expect(
            SceneDraftStore.load(sceneID: "one", defaults: defaults)?[SceneDraftStore.disclosureKey] as? Bool ==
                true
        )
        SceneDraftStore.setSingleSceneID("one", defaults: defaults)
        SceneDraftStore.setSingleSceneHandoff(serialized, defaults: defaults)
        #expect(SceneDraftStore.singleSceneID(defaults: defaults) == "one")
        #expect(SceneDraftStore.singleSceneHandoff(defaults: defaults) != nil)

        SceneDraftStore.discard(sceneIDs: ["other"], defaults: defaults)
        #expect(SceneDraftStore.singleSceneID(defaults: defaults) == "one")
        SceneDraftStore.discard(sceneIDs: ["one"], defaults: defaults)
        #expect(SceneDraftStore.load(sceneID: "one", defaults: defaults) == nil)
    ...
    @Test func keyboardCoordinatorPropagatesTextFocusAndSubmit() {
    ...
    @Test func texturedBackgroundRendersPixelsAtRequestedSize() throws {
        let image = try #require(
            TexturedBackground.renderedImage(size: CGSize(width: 42, height: 28))
        )
        #expect(image.size == CGSize(width: 42, height: 28))
