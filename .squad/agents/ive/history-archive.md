...
- 2026-05-21T19:42:31-07:00: Required-adjustment content can move off the main screen into a native result sheet **only with guardrails**: use SwiftUI `.sheet` with native detents, auto-present only when there is an actual mismatch, keep a compact persistent summary + reopen affordance on the main screen, and provide a visible Close button. Rationale: this preserves the no-vertical-growth constraint on the primary screen without turning Reconcile into a result dead-end or forcing a heavy modal for perfect-match states.
